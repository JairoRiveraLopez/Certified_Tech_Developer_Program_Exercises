...
const baseDeDatos = {
  ...
  const divErrorIngreso = document.querySelector("#error-container");
  const botonIngreso = document.querySelector(".login-btn")
  ...
  baseDeDatos.usuarios.forEach(info => {
    if (info.email === email.value && info.password === password.value ){
      infoIngreso.push(email.value);
      localStorage.setItem('infoUsuarioIngreso', JSON.stringify(infoIngreso));
      divErrorIngreso.remove("p")
    } else {
      divErrorIngreso.classList.remove("hidden")
      divErrorIngreso.innerHTML = `<p><small> La informacion suministrada no coincide con el usuario o contraseña. </small> </p>`
    }
