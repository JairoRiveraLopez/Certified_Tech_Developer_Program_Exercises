...
function renderizarItems() {
    ...
    listadoFelinos.map( felinos => {
        ...
        ` 
        <div class ="item">
        <img src="${felinos.imgUrl}" alt="${felinos.title}" />
        <h2>${felinos.title}</h2>
...
// funcion de cambiar a oscuro

function oscuro(){
let body = document.querySelector("body");
body.classList.toggle("colorFondoOscuro");

let titulos = document.querySelector('h1');
titulos.classList.toggle("tituloFelinos");

let items = document.querySelectorAll('div.item');
    items.forEach(info => {
        info.classList.toggle("atributosItems")
    });

let texto = document.querySelectorAll('h2, p');
    texto.forEach(text => {
        text.classList.toggle("modificacionesTexto")
    })
